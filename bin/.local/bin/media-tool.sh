...
# apt install ffmpeg imagemagick libimage-exiftool-perl whiptail
# pacman -S ffmpeg imagemagick perl-image-exiftool whiptail
...
# --- Media Information Output ---
show_file_info() {
    local sample_file="${TARGET_FILES[0]}"
    local info_text="File: $(basename "$sample_file")\n\n"

    if [ "$BATCH_MODE" -eq 1 ]; then
        info_text="[BATCH MODE - Showing metadata for first file in array]\n$info_text"
    fi

    local info_details=""
    if [ "$FILE_TYPE" == "VIDEO" ] || [ "$FILE_TYPE" == "AUDIO" ]; then
        info_details=$(ffprobe -hide_banner -v error -show_format -show_streams -of default=noprint_wrappers=1:nokey=0 "$sample_file" | grep -E "^(codec_name|width|height|r_frame_rate|sample_rate|channels|duration|bit_rate)=" | sed 's/=/ : /g')
    elif [ "$FILE_TYPE" == "IMAGE" ]; then
        info_details=$(exiftool -s -ImageSize -Megapixels -FileType -MIMEType -ColorSpace "$sample_file")
    fi

    whiptail --title "Media Information" --msgbox "$info_text$info_details" 20 65
}

# --- AUDIO GUI ---
show_audio_main_menu() {
    whiptail \
        --title "Audio TUI - $GUI_TITLE_PREFIX" \
        --menu "Select an operation:" \
        19 55 7 \
        "INFO" "View Media File Information" \
        "CONV" "Convert Format..." \
