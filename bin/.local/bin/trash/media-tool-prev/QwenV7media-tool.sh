		...
		case "$MAIN_CHOICE" in
			...
			CONV)
				while true; do
					...
					case "$SUB_CHOICE" in
						BACK) break ;;
						MP3)  OUTPUT_FILE="${DIR_NAME}/${FILENAME_NO_EXT}_converted.mp3"; execute_command ffmpeg -i "$INPUT_FILE" -q:a 2 "$OUTPUT_FILE"; return 0 ;;
						FLAC) OUTPUT_FILE="${DIR_NAME}/${FILENAME_NO_EXT}_converted.flac"; execute_command ffmpeg -i "$INPUT_FILE" -c:a flac "$OUTPUT_FILE"; return 0 ;;
						WAV)  OUTPUT_FILE="${DIR_NAME}/${FILENAME_NO_EXT}_converted.wav"; execute_command ffmpeg -i "$INPUT_FILE" -c:a pcm_s16le "$OUTPUT_FILE"; return 0 ;;
						OGG)  OUTPUT_FILE="${DIR_NAME}/${FILENAME_NO_EXT}_converted.ogg"; execute_command ffmpeg -i "$INPUT_FILE" -c:a libvorbis -q:a 5 "$OUTPUT_FILE"; return 0 ;;
						M4A)  OUTPUT_FILE="${DIR_NAME}/${FILENAME_NO_EXT}_converted.m4a"; execute_command ffmpeg -i "$INPUT_FILE" -c:a aac -b:a 192k "$OUTPUT_FILE"; return 0 ;;
					esac
				done
				;;
			COMP)
		...
		case "$MAIN_CHOICE" in
			...
			TRANSCODE)
				while true; do
					...
					case "$SUB_CHOICE" in
						BACK) break ;;
						MP4_H264) OUTPUT_FILE="${DIR_NAME}/${FILENAME_NO_EXT}_h264.mp4"; execute_command ffmpeg -i "$INPUT_FILE" -c:v libx264 -preset fast -crf 23 -c:a aac -b:a 128k "$OUTPUT_FILE"; return 0 ;;
						MP4_HEVC) OUTPUT_FILE="${DIR_NAME}/${FILENAME_NO_EXT}_hevc.mp4"; execute_command ffmpeg -i "$INPUT_FILE" -c:v libx265 -preset fast -crf 26 -c:a aac -b:a 128k "$OUTPUT_FILE"; return 0 ;;
						WEBM_VP9) OUTPUT_FILE="${DIR_NAME}/${FILENAME_NO_EXT}_vp9.webm"; execute_command ffmpeg -i "$INPUT_FILE" -c:v libvpx-vp9 -crf 30 -b:v 0 -c:a libopus "$OUTPUT_FILE"; return 0 ;;
						MKV_COPY) OUTPUT_FILE="${DIR_NAME}/${FILENAME_NO_EXT}_remux.mkv"; execute_command ffmpeg -i "$INPUT_FILE" -c copy "$OUTPUT_FILE"; return 0 ;;
					esac
				done
				;;
			COMPRESS)
